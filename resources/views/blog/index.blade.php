    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-16">
        @forelse($posts as $post)
            <article class="flex flex-col group" x-data>
                ...
                <a href="{{ route('blog.show', $post->slug) }}" class="block relative rounded-lg overflow-hidden shadow-lg mb-5">
                    ...
                    {{-- Gradiente sutil sobre la imagen para dar profundidad --}}
                ...
                <div class="flex flex-col flex-grow px-2">
                    ...
                    <h2 class="text-2xl font-bold text-gray-100 leading-tight flex-grow">
                        <a href="{{ route('blog.show', $post->slug) }}" class="bg-left-bottom bg-gradient-to-r from-indigo-400 to-indigo-400 bg-[length:0%_2px] bg-no-repeat group-hover:bg-[length:100%_2px] transition-all duration-500">
                            {{ $post->title }}
                        </a>
                    </h2>
                    ...
                    {{-- Botón de "Leer Más" --}}
                    <div class="mt-6">
                        <a href="{{ route('blog.show', $post->slug) }}" class="inline-flex items-center text-indigo-400 font-semibold hover:text-white transition-colors duration-300">
                            <span>Leer Más</span>
                    ...
                    <div class="sticky top-24 space-y-8">
                        ...
                        <div>
                            ...
                            <ul class="space-y-2 text-gray-400">
                                <li>
                                    <a href="{{ route('blog.index') }}" class="flex justify-between items-center hover:text-white transition-colors">
                                        <span>Todas</span>
                                    </a>
                                </li>
                                @foreach ($categories as $category)
                                    <li>
                                        <a href="{{ route('blog.index', ['category' => $category->id]) }}" class="flex justify-between items-center hover:text-white transition-colors">
                        ...
                        <div>
                            ...
                            <ul class="space-y-2 text-gray-400">
                                @foreach ($archives as $archive)
                                    <li>
                                        <a href="#" class="flex justify-between items-center hover:text-white transition-colors">
