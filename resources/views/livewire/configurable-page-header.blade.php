{{-- resources/views/livewire/configurable-page-header.blade.php --}}
<div>
    <header class="relative text-white bg-gray-900"> {{-- bg-gray-900 como fallback si no hay imagen --}}
        <div class="relative h-[55vh] md:h-[65vh] bg-cover bg-center group"
            ...
            <div class="relative z-10 flex flex-col items-center justify-center h-full text-center px-4 -mt-16 md:-mt-20">

                {{-- Contenedor de texto con animación AOS (como en tu ejemplo) --}}
                <div x-data x-intersect="$el.classList.add('aos-animate')" class="fade-up">
                    <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold tracking-tight leading-tight mb-4 drop-shadow-md">
                        {{ $settings->hero_title ?? 'Título de Cabecera' }} {{-- Accede a través de $settings --}}
                    </h1>
                    <p class="text-lg sm:text-xl md:text-2xl text-gray-200 max-w-3xl drop-shadow-md">
                        {{ $settings->hero_subtitle ?? 'Subtítulo de la cabecera.' }} {{-- Accede a través de $settings --}}
                    </p>
                </div>
            ...
            @if ($isAdmin)
                    <button wire:click="openEditModal" title="Editar Cabecera"
                        class=" right-4 p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transition transform hover:scale-110 z-20">
    ...
    @if ($showEditModal && $isAdmin)
        <div class="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4"
            x-data="{ showModal: @entangle('showEditModal') }" x-show="showModal" x-transition.opacity
            @keydown.escape.window="showModal = false; @this.call('closeEditModal')">
            <div class="bg-gray-800 p-6 md:p-8 rounded-lg shadow-2xl w-full max-w-lg transform transition-all"
                @click.away="showModal = false; @this.call('closeEditModal')">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-2xl font-semibold text-white">Editar Cabecera: <span class="italic font-normal text-lg">"{{ $identifier }}"</span></h3>
                ...
                <form wire:submit.prevent="saveSettings"> {{-- CAMBIO: El método se llama saveSettings --}}
                    <div class="space-y-6">
                        <div>
                            <label for="modalHeroTitle" class="block text-sm font-medium text-gray-300 mb-1">Título Principal:</label>
                        ...
                        <div>
                            <label for="modalHeroSubtitle" class="block text-sm font-medium text-gray-300 mb-1">Subtítulo:</label>
                            ...
                            @error('heroSubtitle') <span class="text-red-400 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label for="modalNewBackgroundImage" class="block text-sm font-medium text-gray-300 mb-1">Imagen de Fondo (Opcional):</label>
                            ...
                            <div wire:loading wire:target="newBackgroundImage" class="text-blue-400 text-xs mt-1">Subiendo imagen...</div>
                            @error('newBackgroundImage') <span class="text-red-400 text-xs mt-1">{{ $message }}</span> @enderror
                    ...
                    <div class="mt-8 flex justify-end space-x-3">
                        ...
                        <button type="submit"
                            ...
                            Guardar Cambios
                            <div wire:loading wire:target="saveSettings, newBackgroundImage" class="inline-block ml-2"> {{-- Actualizado wire:target --}}
                ...
                {{-- Este wire:loading es redundante si el botón ya tiene uno, pero no hace daño --}}
                {{-- <div wire:loading wire:target="saveSettings" class="text-sm text-blue-300">Guardando...</div> --}}
            </div>
        </div>
    @endif
    ...
    @if (session()->has('message'))
        <div x-data="{ showFlash: true }" x-init="setTimeout(() => showFlash = false, 4000)" x-show="showFlash"
            ...
            class="fixed bottom-5 right-5 bg-green-600 text-white py-3 px-6 rounded-lg shadow-xl z-[101] text-sm"> {{-- Aumentado z-index por si acaso --}}
            {{ session('message') }}
        </div>
    @endif
    @if (session()->has('error'))
        <div x-data="{ showFlash: true }" x-init="setTimeout(() => showFlash = false, 4000)" x-show="showFlash"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform translate-y-2"
            class="fixed bottom-5 right-5 bg-red-600 text-white py-3 px-6 rounded-lg shadow-xl z-[101] text-sm">
            {{ session('error') }}
        </div>
    @endif
