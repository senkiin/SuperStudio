...
<div x-data="{ isAdmin: @js($isAdmin) }"> {{-- Añadido x-data --}}
    ...
    {{-- Flash messages para el modal --}}
     @if (session()->has('message'))
        <div class="fixed top-5 right-5 z-[100] w-auto max-w-sm"
             x-data="{ show: true }" x-show="show" x-transition:enter="transform ease-out duration-300 transition" x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2" x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-init="setTimeout(() => show = false, 3500)">
             <div class="flex items-center p-4 text-sm font-medium text-green-800 rounded-lg bg-green-100 dark:bg-gray-800 dark:text-green-400 border border-green-200 dark:border-green-700 shadow-lg" role="alert">
    ...
    @endif
     @if (session()->has('error'))
         <div class="fixed top-5 right-5 z-[100] w-auto max-w-sm"
             x-data="{ show: true }" x-show="show" x-transition:enter="transform ease-out duration-300 transition" x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2" x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0" x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-init="setTimeout(() => show = false, 3500)">
              <div class="flex items-center p-4 text-sm font-medium text-red-800 rounded-lg bg-red-100 dark:bg-gray-800 dark:text-red-400 border border-red-200 dark:border-red-700 shadow-lg" role="alert">
                  <svg class="flex-shrink-0 inline w-4 h-4 me-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20"><path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"/></svg>
    ...
    <div class="relative"> {{-- Contenedor relativo para posicionar botones admin --}}
        ...
        @if ($heroBlock)
            <section class="relative bg-gray-900 text-white overflow-hidden min-h-[60vh] md:min-h-[75vh] flex items-center justify-center">
                {{-- Imagen de Fondo --}}
                @if ($heroBlock->image_path && Storage::disk('public')->exists($heroBlock->image_path))
                    <img src="{{ Storage::url($heroBlock->image_path) }}" alt="{{ $heroBlock->title }}" loading="lazy" class="absolute inset-0 w-full h-full object-cover object-center z-0">
                @else
                ...
                {{-- Contenido Centrado --}}
                <div class="relative z-20 container mx-auto px-4 py-16 text-center max-w-4xl">
                    ...
                    @if ($heroBlock->link_url)
                        <a href="{{ $heroBlock->link_url }}" target="_blank" rel="noopener noreferrer" class="inline-block bg-white text-gray-900 hover:bg-gray-200 text-base md:text-lg font-semibold px-8 py-3 rounded-md shadow-lg transition duration-300 ease-in-out transform hover:scale-105">
    ...
    <x-dialog-modal wire:model.live="showModal">
            ...
            <form wire:submit.prevent="saveHeroBlock" id="heroBlockForm" class="space-y-6">
                ...
                <div>
                 ...
                 <div>
                    ...
                    <x-input id="hero_link_url" type="url" placeholder="https://ejemplo.com" class="mt-1 block w-full" wire:model="form.link_url" />
                    <x-input-error for="form.link_url" class="mt-1.5 text-xs" />
                 </div>
                 <div>
                    <x-label for="hero_link_text" value="{{ __('Texto del Botón') }}" />
                    <x-input id="hero_link_text" type="text" class="mt-1 block w-full" wire:model="form.link_text" />
                 ...
                 <div>
                    ...
                    <div class="mt-4">
                         @if ($photo)
                            ...
                            <img src="{{ $photo->temporaryUrl() }}" class="w-auto h-auto rounded border object-contain max-h-40">
                        @elseif ($current_image_path)
                             <span class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Actual:</span>
                             <img src="{{ Storage::url($current_image_path) }}" class="w-auto h-auto rounded border object-contain max-h-40">
                        @endif
                    </div>
               </div>
               {{-- Checkbox para Activo/Inactivo --}}
                <div class="flex items-center">
                 ...
                 <p class="text-xs text-gray-500 dark:text-gray-400">
                     Nota: Solo se mostrará el bloque activo más reciente. Marcar este como activo puede desactivar otros automáticamente si has añadido esa lógica.
                 </p>
            </form>
        </x-slot>
        ...
        <x-slot name="footer">
            ...
            <x-button type="submit" form="heroBlockForm" class="ml-3" wire:loading.attr="disabled" wire:target="saveHeroBlock, photo">
                 <span wire:loading wire:target="saveHeroBlock, photo" class="inline-flex items-center mr-2"><svg class="animate-spin -ml-1 mr-2 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg></span>
                 <span wire:loading.remove wire:target="saveHeroBlock, photo">{{ $isEditing ? 'Guardar Cambios' : 'Crear Bloque' }}</span>
