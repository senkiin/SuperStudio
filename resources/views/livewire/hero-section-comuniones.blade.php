<div>
    <header class="relative text-white bg-gray-900">
        {{-- CAMBIO: Ruta de imagen por defecto, puede ser la misma si es genérica --}}
        <div class="relative h-[55vh] md:h-[65vh] bg-cover bg-center group"
            style="background-image: url('{{ $settings && $settings->background_image_url ? Storage::url($settings->background_image_url) : '/images/default-hero-image.jpg' }}');">
    ...
    <p class="text-lg sm:text-xl md:text-2xl text-gray-200 max-w-3xl drop-shadow-md fade-up" x-data x-intersect="$el.classList.add('aos-animate')">
        {{ $settings->hero_subtitle ?? 'Subtítulo Hero por Defecto' }}
             ...
             @if ($isAdmin)
                    <button wire:click="openEditModal" title="Editar Sección Hero"
                        class="absolute right-4 p-2 bg-blue-600 hover:bg-blue-700 text-white rounded-full shadow-lg transition transform hover:scale-110 z-20">
        ...
        <div class="fixed inset-0 bg-gray-900 bg-opacity-75 flex items-center justify-center z-50 p-4"
            ...
            <div class="bg-gray-800 p-6 md:p-8 rounded-lg shadow-2xl w-full max-w-lg transform transition-all"
                ...
                <div class="flex justify-between items-center mb-6">
                    <h3 class="text-2xl font-semibold text-white">Editar Sección Hero</h3> {{-- CAMBIO: Título del modal --}}
                ...
                <form wire:submit.prevent="save">
                    <div class="space-y-6">
                        <div>
                            <label for="modalHeroTitleNew" class="block text-sm font-medium text-gray-300 mb-1">Título Principal:</label>
                            <input type="text" wire:model.defer="heroTitle" id="modalHeroTitleNew"
                        ...
                        <div>
                            <label for="modalHeroSubtitleNew" class="block text-sm font-medium text-gray-300 mb-1">Subtítulo:</label>
                            <textarea wire:model.defer="heroSubtitle" id="modalHeroSubtitleNew" rows="4"
                        ...
                        <div>
                            <label for="modalNewBackgroundImageNew" class="block text-sm font-medium text-gray-300 mb-1">Imagen de Fondo:</label>
                            <input type="file" wire:model="newBackgroundImage" id="modalNewBackgroundImageNew"
                            ...
                            @if ($newBackgroundImage)
                                ...
                                <img src="{{ $newBackgroundImage->temporaryUrl() }}" alt="Previsualización" class="mt-2 rounded max-h-40 object-contain">
                            @elseif ($existingBackgroundImageUrl)
                                <p class="text-xs text-gray-400 mt-2">Imagen actual:</p>
                                <img src="{{ Storage::url($existingBackgroundImageUrl) }}" alt="Imagen actual" class="mt-2 rounded max-h-40 object-contain">
                            @else
    ...
    @if (session()->has('message'))
        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show"
            ...
            class="fixed bottom-5 right-5 bg-green-600 text-white py-3 px-6 rounded-lg shadow-xl z-[100] text-sm">
            {{ session('message') }}
        </div>
    @endif
    @if (session()->has('error'))
        <div x-data="{ show: true }" x-init="setTimeout(() => show = false, 4000)" x-show="show"
            x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 transform translate-y-2" x-transition:enter-end="opacity-100 transform translate-y-0"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 transform translate-y-0" x-transition:leave-end="opacity-0 transform translate-y-2"
            class="fixed bottom-5 right-5 bg-red-600 text-white py-3 px-6 rounded-lg shadow-xl z-[100] text-sm">
            {{ session('error') }}
        </div>
    @endif

    {{-- Sección de contenido estático (igual que en tu archivo original) --}}
