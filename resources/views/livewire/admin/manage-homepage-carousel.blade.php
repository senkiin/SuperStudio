...
<x-dialog-modal wire:model.live="showModal" maxWidth="4xl">
    ...
    <x-slot name="content">
        ...
        <div wire:loading.class="opacity-50 pointer-events-none" wire:target="openModal,closeModal,uploadImage,deleteImage,addFromFavorite,toggleActive,updateImageOrder,startEditing,saveEditing,cancelEditing">
                        ...
                        <div wire:sortable.item="{{ $image->id }}" wire:key="carousel-item-{{ $image->id }}"
                            ...
                            <div class="flex items-center p-3 flex-grow space-x-3">
                                ...
                                <div class="flex-grow min-w-0">
                                    @if ($editingImageId === $image->id)
                                        ...
                                        <div class="space-y-2">
                                            <div>
                                                ...
                                                <input type="text" id="edit-caption-{{ $image->id }}" wire:model="editingImageCaption" placeholder="Título (opcional)"
                                            ...
                                            <div>
                                                ...
                                                <input type="url" id="edit-link-{{ $image->id }}" wire:model="editingImageLinkUrl" placeholder="Enlace URL (opcional)"
                                            ...
                                            <div class="flex items-center justify-end space-x-2 pt-1">
                                                <button wire:click="cancelEditing" type="button" class="px-2.5 py-1 text-xs font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-400">Cancelar</button>
                                                <button wire:click="saveEditing" type="button" wire:loading.attr="disabled" wire:target="saveEditing" class="inline-flex items-center px-2.5 py-1 text-xs font-semibold text-white bg-green-600 border border-transparent rounded-md shadow-sm hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-green-500 disabled:opacity-50">
                                                    <svg wire:loading wire:target="saveEditing" class="animate-spin -ml-0.5 mr-1.5 h-3 w-3 text-white" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                    ...
                                    @else
                                        ...
                                        <div class="text-sm">
                                            <p class="font-medium text-gray-800 truncate" title="{{ $image->caption }}">{{ $image->caption ?: '(Sin título)' }}</p>
                                            @if ($image->link_url)
                                                <a href="{{ $image->link_url }}" target="_blank" class="text-xs text-blue-600 hover:underline truncate block max-w-xs" title="{{ $image->link_url }}">{{ $image->link_url }}</a>
                                            @else
                                                <span class="text-xs text-gray-400">(Sin enlace)</span>
                                            @endif
                                            <p class="text-xs text-gray-400 mt-1">Orden: {{ $image->order }} | Estado: {{ $image->is_active ? 'Activa' : 'Inactiva' }}</p>
                                        </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            @if ($editingImageId !== $image->id)
                                <div class="flex items-center space-x-1 sm:space-x-2 p-3 border-t sm:border-t-0 sm:border-l border-gray-200 flex-shrink-0 justify-end">
                                     {{-- Botón Editar --}}
                                     <button wire:click="startEditing({{ $image->id }})" title="Editar título/enlace" class="p-1.5 rounded-md text-gray-500 hover:text-blue-600 hover:bg-blue-100 transition">
                                         <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10"></path></svg>
                                     </button>
                                     {{-- Botón Activar/Desactivar --}}
                                     <button wire:click="toggleActive({{ $image->id }})" title="{{ $image->is_active ? 'Desactivar' : 'Activar' }}" class="p-1.5 rounded-full {{ $image->is_active ? 'hover:bg-gray-200' : 'hover:bg-green-100' }} transition">
                                         @if ($image->is_active)
...
<script>
    function initializeCarouselSortable(elementId) {
         ...
         let checkInterval = setInterval(() => {
            ...
            console.log(`(Check ${checkCount}/${maxChecks}) Checking for Sortable element: #${elementId}`, el); // Log: Verifica si se encuentra
                ...
                if (!el.sortableInstance) {
                    ...
                    el.sortableInstance = Sortable.create(el, {
                        ...
                        onEnd: function (evt) {
                            ...
                            let componentId = el.closest('[wire\\:id]')?.getAttribute('wire:id'); // Safe navigation
                            if (componentId) {
                                ...
                                } else { console.error('Livewire component instance not found for ID:', componentId); }
                            } else { console.error('Could not find parent Livewire component ID.'); }
                        }
                    });
                } else {
        ...
        window.Livewire.on('openCarouselModal', () => {
             console.log('Event openCarouselModal received.'); // Log
              ...
              if (el && el.sortableInstance) {
                    ...
                    } catch(e) { console.error("Error destroying sortable:", e); }
                    el.sortableInstance = null;
              } else {
                console.log('No previous SortableJS instance found to destroy or element not found yet.');
              }
